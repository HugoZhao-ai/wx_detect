...
- 即时交易动作即使标的不明确，也会发送消息并尝试语音呼叫。
- 条件触发或未来预告操作只发送微信消息，不拨号。
- 今天之前的历史操作和历史交易提问不发送消息、不拨号。
...
- `模型判定`：显示 `confidence`、`threshold`、`timing`、`should_notify` 和 `should_call`。
- `should_notify=True`：会发送微信告警消息。
- `should_call=True`：会进一步发起微信语音通话。
- `timing=planned`：预告操作，只会出现 `should_notify=True`。
